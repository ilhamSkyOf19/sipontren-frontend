...
import laodingWhite from "../../../assets/animation/loading-white.svg";
...
const SectionInfo: FC<Props> = memo(({ width }) => {
  // use query
  const { data: dataAktif, isLoading } = useQuery({
    queryKey: ["pendaftaran"],
    queryFn: () => PendaftaranService.getAktif(),
  });

  return (
    <ParallaxGoTop customClass="w-full flex flex-row justify-start items-center px-2 lg:px-6">
      <div className="flex flex-col justify-start items-center w-[98%] min-h-[90vh] pt-28 pb-12 bg-primary-blue gap-2 overflow-hidden md:pt-32 lg:flex-row lg:pt-12 lg:pb-0 lg:items-end  lg:h-[80vh] mb-4 rounded-xl lg:w-full">
        {isLoading ? (
          <div className="w-full h-full flex flex-col justify-center items-center">
            <img src={laodingWhite} alt="loading white" className="w-16" />
          </div>
        ) : dataAktif?.success && dataAktif.data && dataAktif.data.aktif ? (
          <>
            <div className="flex-1 flex flex-col justify-center items-center w-screen gap-3 md:gap-6 lg:items-start lg:pl-8 lg:gap-2 lg:min-h-full">
              ...
              <div className="flex flex-col justify-center items-center w-full gap-2 mb-3 lg:items-start lg:mb-1 lg:mt-2 lg:gap-0">
                ...
                <p className="text-xl text-white font-semibold md:text-3xl lg:text-[2.8rem]">
                  TAHUN AJARAN{" "}
                  {new Date(dataAktif.data.dari.toString()).getFullYear()}/
                  {new Date(dataAktif.data.sampai.toString()).getFullYear()}
                </p>
              </div>
          ...
          <div className="w-full min-h-[90vh] lg:h-full flex flex-col justify-center items-center px-4">
            <h3 className="text-xl lg:text-4xl font-semibold text-primary-white capitalize text-center">
              Mohon maaf <br /> pendaftaran belum dibuka
            </h3>
            <p className="text-primary-white text-sm lg:text-base text-center">
              Silahkan hubungi admin pondok pesantren untuk informasi lebih
              lanjut.
            </p>
          </div>
        )}
